// 案内役の表示: 右下にフクロウの画像（assets/ui/advisor.png）と吹き出し。押すと「いまやるべきこと」と助言の履歴。
...
export function createAdvisorUi(world, reg, settings, { onOpenSettings, onOpen, onView } = {}) {
  ...
  const renderPanel = () => {
    ...
    panel.innerHTML = `<h3>${A.character.name}のひとこと <span style="font-size:11px;opacity:.6;font-weight:normal">${A.character.role}</span></h3>
      <h4>いまやるべきこと</h4>
