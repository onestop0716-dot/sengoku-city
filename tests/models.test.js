...
test('全アセットのコード生成モデルが作れる（頂点数>0、足元は y>=-0.1、足跡内）', async () => {
  ...
  for (const a of reg.assets) {
    ...
    assert.ok(minY >= (a.generator === 'structure' && ['bridge', 'dock'].includes(a.params.kind) ? -0.4 : a.generator === 'vehiclePart' ? -0.5 : -0.1), `${a.id}: 地面より下 ${minY}`);
    ...
    const size = a.generator === 'houseNoble' ? 1.5 : a.generator === 'houseShi' ? 1.0 : a.generator === 'structure' ? (big[a.params.kind] ?? 0.6) : a.generator === 'militaryBuilding' ? 1.0 : a.generator === 'agent' || a.generator === 'personStatic' || a.generator === 'vehiclePart' ? 1.2 : a.generator === 'tree' ? 0.9 : 0.6;
